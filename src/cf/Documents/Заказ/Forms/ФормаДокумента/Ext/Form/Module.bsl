...
#КонецЕсли   
//++ Task-1: Запрет редактирования закрытого заказа, кроме полных прав Шихарбеева 12.01.25
Если Объект.СостояниеЗаказа= ПредопределенноеЗначение("Перечисление.СостоянияЗаказов.Закрыт") И 
	(НЕ РольДоступнаНаСервере("Администратор")ИЛИ НЕ РольДоступнаНаСервере("РедактированиеЗакрытогоЗаказа")) тогда
